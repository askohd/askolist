...
export async function POST(request: Request) {
  try {
    ...
    if (!reportType || !reportId || !action) {
      return redirectToAdmin(request, "error=missing_report_action");
    }
    ...
    const handledData = {
      ...
      handled_by_username: staff.username || "Staff",
    ...
    if (reportType === "server") {
      ...
      if (action === "lock_reported_server") {
        if (!serverId) return redirectToAdmin(request, "error=no_server");
      ...
      if (action === "ban_reported_server") {
        if (!isAdmin) return redirectToAdmin(request, "error=admin_required");
        if (!serverId) return redirectToAdmin(request, "error=no_server");
    ...
    if (reportType === "review") {
      ...
      if (action === "hide_review") {
        if (!reviewId) return redirectToAdmin(request, "error=no_review");
    ...
    return redirectToAdmin(request, "error=unknown_report_action");
  } catch (error) {
